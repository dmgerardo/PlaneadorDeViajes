// Vista "Ciudades": asigna la ciudad en la que se está cada día del viaje,
...
// (ciudadPorDia) es la que usan las vistas Calendario y Agenda para saber
...
async function montarVistaCiudades(contenedor, tripId, sesion) {
  ...
  function render() {
    ...
    limpiar(wrap);
    if (dias.length === 0) {
      wrap.innerHTML = '<p style="font-size:12px;color:var(--color-texto-suave)">Primero captura las fechas del viaje en la pestaña Generales.</p>';
    }
    dias.forEach(dia => {
    ...
    if (Object.keys(estado.ciudades).length === 0) {
      chipsEl.innerHTML = '<p style="font-size:12px;color:var(--color-texto-suave)">Primero agrega ciudades en la pestaña Generales.</p>';
      return;
    }
